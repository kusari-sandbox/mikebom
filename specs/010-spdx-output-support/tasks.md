...
- [ ] T018 [P] [US1] `SpdxId` newtype in `mikebom-cli/src/generate/spdx/ids.rs` per `data-model.md` §3.1: `for_purl(&Purl) -> SpdxId` (R7 derivation: `SPDXRef-Package-<base32(SHA-256(canonical_purl))[..16]>`), `document() -> SpdxId` const for `SPDXRef-DOCUMENT`, transparent serde, no public string constructor; include `#[cfg(test)] #[cfg_attr(test, allow(clippy::unwrap_used))] mod tests` covering deterministic PURL→ID, character-set conformance, and uniqueness across a 10k-PURL fixture
- [ ] T019 [P] [US1] `SpdxDocumentNamespace` newtype in `mikebom-cli/src/generate/spdx/document.rs` per `data-model.md` §3.2: `derive(&ScanResult) -> SpdxDocumentNamespace` (R8 derivation: `https://mikebom.kusari.dev/spdx/<base32(SHA-256(canonical_scan_inputs))[..32]>` where canonical_scan_inputs = scan target + mikebom version + sorted set of resolved component PURLs), transparent serde, with unit tests asserting determinism for identical inputs and divergence for different ones
- [ ] T020 [P] [US1] SPDX 2.3 envelope structs in `mikebom-cli/src/generate/spdx/document.rs`: `SpdxDocument`, `CreationInfo`, `SpdxAnnotation`, `SpdxAnnotationType` enum, `SpdxExternalDocumentRef` per `data-model.md` §3.3 with `serde::Serialize` derive and field-rename attributes
- [ ] T021 [P] [US1] SPDX 2.3 Package + license + checksum + externalRef structs in `mikebom-cli/src/generate/spdx/packages.rs`: `SpdxPackage`, `SpdxChecksum`, `SpdxChecksumAlgorithm` enum, `SpdxLicenseField` enum (untagged, with custom Serialize that emits literal `"NOASSERTION"`/`"NONE"` for sentinels), `SpdxExternalRef`, `SpdxExternalRefCategory` enum
- [ ] T022 [P] [US1] SPDX 2.3 Relationship struct + mapping in `mikebom-cli/src/generate/spdx/relationships.rs`: `SpdxRelationship`, `SpdxRelationshipType` enum (`DESCRIBES`, `DEPENDS_ON`, `DEV_DEPENDENCY_OF`, `CONTAINS`, `CONTAINED_BY`), and the `Relationship.kind → SpdxRelationshipType` mapping table from `data-model.md` §3.4
- [ ] T023 [US1] `pub fn build_packages(scan: &ScanResult) -> Vec<SpdxPackage>` in `mikebom-cli/src/generate/spdx/packages.rs`: for each `ResolvedComponent` emit one `SpdxPackage` with `SPDXID = SpdxId::for_purl(&c.purl)`, name/version, `externalRefs[PACKAGE-MANAGER/purl]` carrying the canonical PURL string, every hash → `checksums[]` entry, declared and concluded licenses canonicalized via `spdx::Expression::canonicalize` (NOASSERTION on parse failure), `supplier`/`originator` as `"Organization: <name>"` / `"Person: <name>"` or `NOASSERTION`, `downloadLocation = NOASSERTION` (US2 may refine), `filesAnalyzed = false` (depends on T018, T021)
- [ ] T024 [US1] `pub fn build_relationships(scan: &ScanResult, root: &SpdxId) -> Vec<SpdxRelationship>` in `mikebom-cli/src/generate/spdx/relationships.rs`: emit `DESCRIBES` from `SPDXRef-DOCUMENT` to root, map every `Relationship.kind` per the table from T022, expand CycloneDX nested-component containment into explicit `CONTAINS`/`CONTAINED_BY` relationships between flat Packages (FR-010, FR-011) (depends on T018, T022)
- [ ] T025 [US1] `pub fn build_document(scan: &ScanResult, cfg: &OutputConfig) -> SpdxDocument` in `mikebom-cli/src/generate/spdx/document.rs`: assemble `spdxVersion = "SPDX-2.3"`, `dataLicense = "CC0-1.0"`, `SPDXID = SPDXRef-DOCUMENT`, `name = <scan target description>`, `documentNamespace = SpdxDocumentNamespace::derive(scan)`, `creationInfo = { created: cfg.created (RFC3339), creators: ["Tool: mikebom-<version>"] }`, `packages = build_packages(scan)`, `relationships = build_relationships(scan, &root)`, `documentDescribes = vec![root]`; synthesize a deterministic root Package when no natural root exists (Edge Case "Multiple roots / no root") (depends on T019, T020, T023, T024)
- [ ] T026 [US1] `Spdx2_3JsonSerializer` impl `SbomSerializer` in `mikebom-cli/src/generate/spdx/mod.rs`: `id() == "spdx-2.3-json"`, `default_filename() == "mikebom.spdx.json"`, `experimental() == false`, `serialize()` calls `build_document` then serializes via `serde_json::to_vec_pretty` honoring the determinism contract from `data-model.md` §8 (use `BTreeMap`/sorted iteration; never `HashMap`); register in `SerializerRegistry::with_defaults()` in `mikebom-cli/src/generate/mod.rs` (depends on T025)
- [ ] T027 [US1] Update `--help` text and unknown-id error message in `mikebom-cli/src/sbom/scan_cmd.rs` to list `spdx-2.3-json` as an accepted format and to mention it in the registered-ids enumeration (depends on T026)
...
- [ ] T032 [P] [US2] Doc-completeness CI check in `mikebom-cli/tests/sbom_format_mapping_coverage.rs`: walks the pinned CDX golden fixtures from T010, extracts every distinct `properties[].name` value and every distinct field path emitted, then asserts that `docs/reference/sbom-format-mapping.md` (a) contains a row whose CycloneDX-location column matches each one AND (b) every row has a non-empty value in all three format columns (CycloneDX, SPDX 2.3, SPDX 3) — entries like `omitted — <reason>` or `defer — <reason>` count as non-empty; literal whitespace, `TODO`, `TBD`, or `?` markers fail the check (SC-004 — prevents map drift as new milestones add fields, in either format column)
