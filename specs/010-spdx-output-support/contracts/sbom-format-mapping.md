...
**Version**: v1 | **Owner**: milestone 010 | **Canonical home**: `docs/reference/sbom-format-mapping.md` (this file under `contracts/` mirrors it for plan-review purposes)
...
|---|--------------|------------------------|-------------------|---------------------|---------------|
| A1 | PURL | `/components/{i}/purl` | `/packages/{i}/externalRefs[]` with `referenceCategory: "PACKAGE-MANAGER"`, `referenceType: "purl"`, `referenceLocator: <PURL>` | `Package/externalRef` with `externalRefType: "purl"`, `locator: <PURL>` | All three formats have native, identical PURL semantics. |
| A2 | name | `/components/{i}/name` | `/packages/{i}/name` | `Package/name` | Native in all. |
| A3 | version | `/components/{i}/version` | `/packages/{i}/versionInfo` | `Package/packageVersion` | Native in all. |
| A4 | supplier (org) | `/components/{i}/supplier/name` | `/packages/{i}/supplier` (`"Organization: <name>"` or `"NOASSERTION"`) | `Package/suppliedBy` (Agent element ref) | Native in all; SPDX requires `NOASSERTION` when unknown — never omit. |
| A5 | author (person) | `/components/{i}/author` | `/packages/{i}/originator` (`"Person: <name>"` or `"NOASSERTION"`) | `Package/originatedBy` (Agent element ref) | Native in all. |
| A6 | hashes | `/components/{i}/hashes[]` (each: `{alg, content}`) | `/packages/{i}/checksums[]` (each: `{algorithm, checksumValue}`) | `Package/verifiedUsing[]` of `Hash` elements with `algorithm` + `hashValue` | Native in all; mapping is 1-1. |
| A7 | license — declared | `/components/{i}/licenses[]` with `license.id` or `expression` | `/packages/{i}/licenseDeclared` (canonical SPDX expression, `"NOASSERTION"`, or `"NONE"`) | `Package/declaredLicense` (LicenseExpression element) | Native in all; canonicalize via `spdx::Expression::canonicalize` before emitting. |
| A8 | license — concluded | `/components/{i}/licenses[]` (concluded marker) | `/packages/{i}/licenseConcluded` | `Package/concludedLicense` | Native in all. |
