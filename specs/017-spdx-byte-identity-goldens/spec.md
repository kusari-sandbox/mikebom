...
**Independent Test**: After this story ships, `rg -l 'env\("HOME"' mikebom-cli/tests/` finds 0 inline call sites — every test that sets `HOME` does so via `common::apply_fake_home_env`. A grep over the test tree for `env\("M2_REPO"|GOPATH|GOMODCACHE|CARGO_HOME` likewise returns 0 inline sites.
...
- **FR-006**: A new module `mikebom-cli/tests/common/normalize.rs` MUST export at least the following public items: `normalize_cdx_for_golden(raw: &str, workspace: &Path) -> String`, `normalize_spdx23_for_golden(doc: serde_json::Value, workspace: &Path) -> serde_json::Value`, `normalize_spdx3_for_golden(doc: serde_json::Value, workspace: &Path) -> serde_json::Value`, and `apply_fake_home_env(cmd: &mut std::process::Command, fake_home: &Path)`. The module-doc MUST explain the *why* of each masking step (cite the spec or the leak vector).
...
- **FR-008**: Every test under `mikebom-cli/tests/*.rs` that today calls `Command::new(...).env("HOME", ...)` MUST migrate to `common::apply_fake_home_env`. Verified: `rg 'env\("HOME"' mikebom-cli/tests/ -g '!common/'` returns 0 hits after the migration.
...
- **FR-010**: The byte-identity goldens MUST tolerate the legitimate run-scoped fields by masking exactly: SPDX 2.3 — `creationInfo.created`; SPDX 3 — every `CreationInfo.created` graph element AND, if document IRI is run-scoped in any future emitter path, the IRI itself. The set of masked fields MUST be documented in `tests/common/normalize.rs` module-doc with rationale per field.
...
- **SC-004**: `rg 'env\("HOME"' mikebom-cli/tests/ -g '!common/'` returns 0 hits after the migration. Verified by the grep itself.
