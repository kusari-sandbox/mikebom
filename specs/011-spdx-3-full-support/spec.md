...
- SC-001's SPDX 3 scoring branch is **gracefully skipped** while upstream sbomqs does not yet parse SPDX 3.0.1. sbomqs v2.0.6 (the CI-pinned version) reports `Unsupported SBOM specification` for SPDX 3 documents; the test harness detects that shape and emits a visible per-ecosystem diagnostic instead of failing. CDX ↔ SPDX 2.3 SC-001 enforcement remains active throughout the skip period. When sbomqs adds a SPDX 3 reader, the skip path disappears and SC-001 enforcement for SPDX 3 activates automatically without a code change.
