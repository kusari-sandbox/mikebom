...
- [X] T028 [P] [US2] Unit test `sign_local_canonical_payload_is_deterministic` at `attestation::signer::tests`: sign twice, assert byte-identical payload bytes.
- [~] T029 [P] [US2] Negative case covered — `load_local_signer_passphrase_env_missing_errors` exercises missing env var → `KeyPassphraseInvalid`. Full encrypted-PEM positive case deferred to a follow-on fixture task (needs a pre-generated encrypted PEM to land in `tests/fixtures/`; in-process encrypted-PEM generation requires `openssl` helpers sigstore 0.10 doesn't expose).
- [X] T030 [P] [US2] Integration test scaffold landed as `mikebom-cli/tests/verify_dsse.rs` (consolidates sign + verify; keypair generated in-process via sigstore scheme::create_signer).

### Implementation for User Story 2

- [X] T031 [US2] `load_local_signer(path, passphrase_env)` in `attestation::signer`: reads PEM, optionally decrypts via env-var passphrase, returns `SigStoreKeyPair`. Errors → `KeyFileMissing` / `KeyPassphraseInvalid` / `KeyParseError`.
- [X] T032 [US2] `OidcProvider::detect()` in `attestation::signer`: GitHub Actions env vars → `GitHubActions`; `SIGSTORE_ID_TOKEN` → `Explicit`; fallback → `Interactive`.
- [~] T033 [US2] `sign_keyless` ships as a typed skeleton: the full OIDC → Fulcio → Rekor flow requires live network calls. v1 returns `SigningError::OidcTokenError` with a descriptive detail; the CLI plumbing (flag parsing, identity construction, error routing) all works end-to-end. Full Fulcio/Rekor integration deferred to a follow-on task (needs real OIDC token plumbing + integration tests gated on `SIGSTORE_ID_TOKEN`).
- [X] T034 [US2] `sign_local(stmt, keypair)` in `attestation::signer`: canonicalize payload, PAE, sign via `SigStoreSigner`, emit `SignedEnvelope` with embedded public PEM + `KeyAlgorithm`.
- [X] T035 [US2] Unified `sign(stmt, identity) -> Result<Option<SignedEnvelope>, SigningError>`. Returns `Ok(None)` for `SigningIdentity::None` (caller emits raw), `Ok(Some(envelope))` for real signing paths, typed error on hard-fail.
- [X] T036 [US2] New `write_attestation_signed(stmt, path, identity)` in `attestation::serializer`: calls `sign()`, emits envelope JSON on success, falls through to legacy raw shape on `None` (with FR-004 warning), hard-fails no-file-written on `SigningError`.
- [X] T037 [US2] Added all 7 signing flags to `ScanArgs` per contract: `--signing-key`, `--signing-key-passphrase-env`, `--keyless`, `--fulcio-url`, `--rekor-url`, `--no-transparency-log`, `--require-signing`. `ScanArgs::build_signing_identity()` produces a typed `SigningIdentity` with mutual-exclusion + require-signing validation.
- [X] T038 [US2] `scan.rs::execute_scan` (Linux path) now calls `build_signing_identity()` then `write_attestation_signed()` — hard-fail via `anyhow::Error` propagation kills the process with non-zero exit.
- [X] T039 [US2] `RunArgs` mirrors the same 7 flags and forwards them verbatim into the composed `ScanArgs` in `run.rs::execute`.
- [X] T040 [US2] `sign_local_roundtrips_through_verifier` in `attestation::signer::tests` + `cli_verifies_local_key_signed_envelope` in `mikebom-cli/tests/verify_dsse.rs`: in-process keypair → sign → parse back → verify → Pass. Both in-crate and cross-process paths covered.
- [X] T041 [US2] `sign_local_canonical_payload_is_deterministic` in `attestation::signer::tests`: two calls with the same statement → byte-identical base64 payload.
- [X] T042 [US2] `sign_hard_fails_on_missing_key_file` in `attestation::signer::tests` covers the core hard-fail contract. `cli_fails_hard_with_require_signing_and_no_key` in `mikebom-cli/tests/verify_dsse.rs` confirms the CLI surfaces `--require-signing` + `--signing-key` + `--keyless` flags.
- [~] T043 [US2] Full keyless CI integration test deferred until T033's real Fulcio/Rekor flow lands. The flag plumbing that makes the test meaningful is all in place.
