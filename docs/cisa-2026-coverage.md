...
# 2. Sign at scan time.
#    NOTE (m777): keyless signing is refused for CycloneDX output — a
#    Sigstore Bundle has no conformant JSON Signature Format
#    representation, so waybill will not emit a CycloneDX document it
#    knows to be schema-invalid. Use an SPDX format, which signs to a
#    detached sidecar, or use --sign-key for CycloneDX.
waybill sbom scan --path <target> \
    --format spdx-2.3-json --output /tmp/signed.spdx.json \
    --sign
