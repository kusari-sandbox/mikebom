...
| A7 | license — declared | `/components/{i}/licenses[]` with `license.id` or `expression` | `/packages/{i}/licenseDeclared` (canonical SPDX expression, `"NOASSERTION"`, or `"NONE"`) | `Relationship` element with `relationshipType: "hasDeclaredLicense"`, `from: <Package IRI>`, `to: [<LicenseExpression IRI>]`; the LicenseExpression is a separate `simplelicensing_LicenseExpression` element carrying `simplelicensing_licenseExpression: "<canonical SPDX expr>"`. LicenseExpression elements are deduped across packages. | Native in 1.6/2.3; SPDX 3 expresses licenses via a Relationship edge to a first-class LicenseExpression element. |
| A8 | license — concluded | `/components/{i}/licenses[]` (concluded marker) | `/packages/{i}/licenseConcluded` | Same shape as A7 with `relationshipType: "hasConcludedLicense"`. **Omitted when concluded expression equals declared expression** (no redundant edge — mirrors the SPDX 2.3 path's defensive-omission rule). | Native in 1.6/2.3; SPDX 3 mirrors A7's shape. |
